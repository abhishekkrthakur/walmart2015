...
with open('../fulldata_company_bought.libsvm', 'w') as f:
...
visit = np.unique(data.VisitNumber.values)
with open('../fulldata_company_count.libsvm', 'w') as f:
    for k, v in enumerate(visit):
        trip = data[data['VisitNumber'] == v]
        trip = trip.drop('VisitNumber', axis=1)
        if trip.empty:
...
data = pd.read_csv('../data/full_data_enc.csv', usecols=['VisitNumber', 'ScanCount', 'company'])
visit = np.unique(data.VisitNumber.values)
with open('../fulldata_company_return.libsvm', 'w') as f:
    for k, v in enumerate(visit):
        trip = data[data['VisitNumber'] == v]
        trip = trip.drop('VisitNumber', axis=1)
        if trip.empty:
...
visit = np.unique(data.VisitNumber.values)
with open('../fulldata_company_per_trip.libsvm', 'w') as f:
